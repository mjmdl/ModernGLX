...
#define GL_LIST								\
    GLE(void, glAttachShader, GLuint, GLuint);				\
    GLE(void, glBindBuffer, GLenum, GLuint);				\
    GLE(void, glBindFramebuffer, GLenum, GLuint);			\
    GLE(void, glBindVertexArray, GLuint);				\
    GLE(void, glBufferData, GLenum, GLsizeiptr, const GLvoid *, GLenum); \
    GLE(void, glBufferSubData, GLenum, GLintptr, GLsizeiptr, const GLvoid *); \
    GLE(GLenum, glCheckFramebufferStatus, GLenum);			\
    GLE(void, glClearBufferfv, GLenum, GLint, const GLfloat *);		\
    GLE(void, glCompileShader, GLuint);					\
    GLE(GLuint, glCreateProgram, void);					\
    GLE(GLuint, glCreateShader, GLenum);				\
    GLE(void, glDeleteBuffers, GLsizei, const GLuint *);		\
    GLE(void, glDeleteFramebuffers, GLsizei, const GLuint *);		\
    GLE(void, glDeleteProgram, GLuint);					\
    GLE(void, glDeleteShader, GLuint);					\
    GLE(void, glDetachShader, GLuint, GLuint);				\
    GLE(void, glDrawBuffers, GLsizei, const GLenum *);			\
    GLE(void, glEnableVertexAttribArray, GLuint);			\
    GLE(void, glFramebufferTexture2D, GLenum, GLenum, GLenum, GLuint, GLint); \
    GLE(void, glGenBuffers, GLsizei, GLuint *);				\
    GLE(void, glGenFramebuffers, GLsizei, GLuint *);			\
    GLE(void, glGenVertexArrays, GLsizei, GLuint *);			\
    GLE(GLint, glGetAttribLocation, GLuint, const GLchar *);		\
    GLE(void, glGetProgramInfoLog, GLuint, GLsizei, GLsizei *, GLchar *); \
    GLE(void, glGetProgramiv, GLuint, GLenum, GLint *);			\
    GLE(void, glGetShaderInfoLog, GLuint, GLsizei, GLsizei *, GLchar *); \
    GLE(void, glGetShaderiv, GLuint, GLenum, GLint *);			\
    GLE(GLint, glGetUniformLocation, GLuint, const GLchar *);		\
    GLE(void, glLinkProgram, GLuint);					\
    GLE(void, glShaderSource, GLuint, GLsizei count, const GLchar *const *, \
	const GLint *);							\
    GLE(void, glUniform1i, GLint, GLint);				\
    GLE(void, glUniform1f, GLint, GLfloat);				\
    GLE(void, glUniform2f, GLint, GLfloat, GLfloat);			\
    GLE(void, glUniform4f, GLint, GLfloat, GLfloat, GLfloat, GLfloat);	\
    GLE(void, glUniformMatrix4fv, GLint, GLsizei, GLboolean, const GLfloat *); \
    GLE(void, glUseProgram, GLuint);					\
    GLE(void, glVertexAttribPointer, GLuint, GLint, GLenum, GLboolean, GLsizei, \
	const GLvoid *);
...
#define GLE(RET, NAME, ...)						\
    NAME = (NAME##proc *)glXGetProcAddressARB(#NAME);			\
    if (NAME == NULL) {							\
